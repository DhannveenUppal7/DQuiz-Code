...
function QuizResult({ quizData }) {
  ...
  const copyResult = () => {
    ...
    el.value = `Hello I am Glad that I can share my DQuiz Result with you....\nQuiz Attempted By: ${quizData.playData.aName}\nQuiz Name: ${quizData.playData.qName}\nQuiz Subject: ${quizData.playData.qSubject}Percentage: ${quizData.playData.qPercentage}%\nYou too can play this quiz at the link - \n${window.location.origin}/quizstart/${quizData.playData.id}`;
  ...
  useEffect(() => {
    if ((quizData.playData.qScore) >= 5) {
      ...
      certificateImg.onload = () => {
        ...
        context.fillText(quizData.playData.aName, 380, 275);
        context.font = "23px Tahoma"
        context.fillText(`Quiz: ${quizData.playData.qName}`, 390, 408)
  ...
  const downloadCertificate = () => {
    if ((quizData.playData.qScore) >= 5) {
      ...
      cf.download = `Certificate_${quizData.playData.aName}.png`;
      cf.href = image;
  ...
  return (
    <div>
      {quizData ? <div style={{ textAlign: "center" }}>
        ...
        <h4 className='notes'>Attempted By: {quizData.playData.aName}</h4>
        <h4 className='notes'>Quiz Name: {quizData.playData.qName}</h4>
        <h4 className='notes'>Quiz Subject: {quizData.playData.qSubject}</h4>
        ...
        <br />
        <Link className="btn btn-primary" to={`/quizstart/${quizData.playData.id}`} state={{}}>Play the Quiz Again</Link>
        <hr />
        ...
        <h2 style={{ marginTop: "30px", color: "black" }}>Answer Corrections</h2>
